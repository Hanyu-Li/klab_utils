...
def large_local_to_cloud(data_path, cloud_path, begin=None, end=None, dtype=None, multi=False, z_step=64,
  ...
  data_generator = large_data_generator(
      data_path, S, S+L, z_step, data_type, multi)
...
def main():
  ...
  if args.image is not None:
    ...
    if not args.large:
      ...
      local_to_cloud(image, image_cloud_path, layer_type='image',
                     resolution=resolution, scale=args.scale)
    else:
  ...
  if args.labels is not None:
    ...
    if not args.large:
      ...
      local_to_cloud(labels, labels_cloud_path, layer_type='segmentation',
                     resolution=resolution, scale=args.scale)
    else:
