...
class EM_preprocessor(object):
  ...
  def test_one_image(self):
    ...
    f_dummy = glob.glob(os.path.join(self.input_dir, 'Tile_*.tif'))[0]
  ...
  def prepare_align_txt(self):
    with open(self.output, 'w') as f_out:
      for f in self.flist:
        ...
        res = re.search(r'Tile_(\d+)-(\d+)-(\d+)_0-000.s(\d+)_e00.tif', f)
  ...
  def run(self):
    ...
    self.flist = glob.glob(os.path.join(self.input_dir, 'Tile_*.tif*'))
    def get_index(f): 
      return re.search(r's([0-9]+)_', os.path.basename(f)).group(1)
